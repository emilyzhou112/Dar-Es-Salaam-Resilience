/* Select points that represent water amenities from OSM  */
...
/* Select polygons that represent water amenities from OSM  */
...
/* Select points that represent residential buildings from OSM  */
...
/* Select polygons that represent residential buildings from OSM  */
...
/* Create centriod for each feature in waterpoly
   Combine the waterpoly table and the waterpoint table together*/
...
/* Comparing water amenity to flood
...
/*
Create centroid for each feature in respoly
Combine respoly and respoint 
*/
CREATE TABLE res AS
...
/* How many households in each ward have difficulties accessing clean water during flood?*/
CREATE TABLE water_access_wards AS
SELECT ward_census.ward_name, ward_census.ward_code, water_accessibility.isflood, water_accessibility.building
FROM water_accessibility 
...
SELECT ward_name, ward_code, count(isflood) AS flood_houses, count(building) AS total_houses
FROM water_access_wards
GROUP BY ward_name, ward_code
...
ON ward_census.ward_name = wards_water_summary.ward_name AND ward_census.ward_code = wards_water_summary.ward_code


----------------

/* thought process for vpoly*/
CREATE TABLE service_area AS
SELECT st_dump(
         st_voronoipolygons(
            st_collect(geom),
            0.0, 
            (SELECT st_union(geom) FROM wards) 
         )
       ).geom as geom
